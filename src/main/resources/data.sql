INSERT INTO bank (code, name) SELECT 'D001', '국민은행' FROM DUAL WHERE NOT EXISTS (SELECT * FROM bank WHERE code = 'D001');
INSERT INTO bank (code, name) SELECT 'D002', '신한은행' FROM DUAL WHERE NOT EXISTS (SELECT * FROM bank WHERE code = 'D002');
INSERT INTO bank (code, name) SELECT 'D003', '하나은행' FROM DUAL WHERE NOT EXISTS (SELECT * FROM bank WHERE code = 'D003');
...
INSERT INTO user (name) SELECT '김민수' FROM DUAL WHERE NOT EXISTS (SELECT * FROM user WHERE name = '김민수');
INSERT INTO user (name) SELECT '장민지' FROM DUAL WHERE NOT EXISTS (SELECT * FROM user WHERE name = '장민지');
